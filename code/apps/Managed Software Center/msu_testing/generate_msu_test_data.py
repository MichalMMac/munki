...
def main():
    ...
    install_info = plistlib.readPlist(
        os.path.join(thisdir, 'InstallInfo.plist'))
    ...
    apple_updates = plistlib.readPlist(
        os.path.join(thisdir, 'AppleUpdates.plist'))
    # get_random(apple_updates['AppleUpdates'])
    apple_updates['AppleUpdates'] = []
    plistlib.writePlist(
        apple_updates,
        '/Library/Managed Installs/AppleUpdates.plist')

    self_service = plistlib.readPlist(
        os.path.join(thisdir, 'SelfServeManifest'))
